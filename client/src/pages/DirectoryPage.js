...
const DirectoryPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDirectories = async () => {
        ...
        if (!error) {
          setDirectories(data); // Directly use the fetched data
        } else {
          console.error(error);
        }
      };
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Buttons to navigate to Root and Upload Directory Pages */}
        <div className="mb-4">
            <Link
            to="/root" // Navigate to RootDirectoryPage
            className="bg-gray-500 hover:bg-gray-600 text-white font-medium px-4 py-2 rounded-md shadow transition duration-300 mr-4"
            >
            Root Directory
            </Link>
            <Link
            to="/uploads" // Navigate to UploadDirectoryPage
        ...
        {directories.length > 0 ? (
            ...
            {directories.map((dir) => (
                ...
                <Link
                    to={`/directory/${dir.directoryId}`} // Navigate to DirectoryPage for each directory
